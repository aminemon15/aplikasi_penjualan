...
class Service extends MY_Controller 
{
	...
	public function service()
	{
		...
		if($level == 'admin' OR $level == 'kasir')
		{
				...
				if( ! empty($_POST['kode_barang']))
				{
					...
					if($total > 0)
					{
						...
						if($this->form_validation->run() == TRUE)
						{
							...
							{
								$this->load->model('m_service_master');
								$master = $this->m_service_master->insert_master($nomor_nota, $tanggal, $id_kasir, $id_pelanggan, $id_barang, $bayar, $grand_total, $catatan);
								if($master)
								{
									$id_master 	= $this->m_service_master->get_id($nomor_nota)->row()->id_service_m;
									$inserted	= 0;

									$this->load->model('m_service_detail');
									...
									foreach($_POST['kode_barang'] as $k)
									{
										if( ! empty($k))
										{
											...
											$insert_detail	= $this->m_service_detail->insert_detail($id_master, $id_barang, $jumlah_beli, $harga_satuan, $sub_total);
											if($insert_detail)
											{
			...
			else
			{
				...
				$this->load->view('service/service', $dt);
			}
		}
	}
	...
	{
		$this->load->model('m_service_master');
		$cek = $this->m_service_master->cek_nota_validasi($nota);
	...
	public function history()
	{
		...
		{
			$this->load->view('service/transaksi_history');
		}
	}
	...
	public function history_json()
	{
		...
		$fetch			= $this->m_service_master->fetch_data_service($requestData['search']['value'], $requestData['order'][0]['column'], $requestData['order'][0]['dir'], $requestData['start'], $requestData['length']);
		...
		{ 
			...
			$nestedData[]	= "<a href='".site_url('service/detail-transaksi/'.$row['id_service_m'])."' id='LihatDetailTransaksi'><i class='fa fa-file-text-o fa-fw'></i> ".$row['nomor_nota']."</a>";
			...
			{
				$nestedData[]	= "<a href='".site_url('service/hapus-transaksi/'.$row['id_service_m'])."' id='HapusTransaksi'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
	...
	public function detail_transaksi($id_service)
	{
		if($this->input->is_ajax_request())
		{
			$this->load->model('m_service_detail');
			...
			$dt['detail'] = $this->m_service_detail->get_detail($id_service);
			$dt['master'] = $this->m_service_master->get_baris($id_service)->row();
			
			$this->load->view('service/transaksi_history_detail', $dt);
		}
	}

	public function hapus_transaksi($id_service)
	{
		if($this->input->is_ajax_request())
		{
			$level 	= $this->session->userdata('ap_level');
			if($level == 'admin')
			{
				...
				$nota 	= $this->m_service_master->get_baris($id_service)->row()->nomor_nota;
				$hapus 	= $this->m_service_master->hapus_transaksi($id_service, $reverse_stok);
				if($hapus)
				{
	...
	public function pelanggan()
	{
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			$this->load->view('service/pelanggan_data');
		}
	}
	...
	public function pelanggan_json()
	{
		...
		{ 
			...
			if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan') 
			{
				$nestedData[]	= "<a href='".site_url('service/pelanggan-edit/'.$row['id_pelanggan'])."' id='EditPelanggan'><i class='fa fa-pencil'></i> Edit</a>";
			}

			if($level == 'admin') 
			{
				$nestedData[]	= "<a href='".site_url('service/pelanggan-hapus/'.$row['id_pelanggan'])."' id='HapusPelanggan'><i class='fa fa-trash-o'></i> Hapus</a>";
			}
	...
	public function tambah_pelanggan()
	{
		...
		if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
		{
			...
			{
				$this->load->view('service/pelanggan_tambah');
			}
		}
	}
			...
			if($level == 'admin' OR $level == 'kasir' OR $level == 'keuangan')
			{
				if($this->input->is_ajax_request())
				{
					...
					else
					{
						...
						$this->load->view('service/pelanggan_edit', $dt);
					}
				}
			}
		}
