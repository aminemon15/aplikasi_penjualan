<?php
class M_service_master extends CI_Model
{
	function insert_master($nomor_nota, $tanggal, $id_kasir, $id_pelanggan, $id_barang, $bayar, $grand_total, $catatan)
	{
		$dt = array(
		...
		return $this->db->insert('pj_service_master', $dt);
	}
	...
	function hapus_transaksi($id_service, $reverse_stok)
	{
		if($reverse_stok == 'yes'){
			$loop = $this->db
				->select('id_barang, jumlah_beli')
				->where('id_service_m', $id_service)
				->get('pj_service_detail');
		...
		$this->db->where('id_service_m', $id_service)->delete('pj_service_detail');
		return $this->db
			->where('id_service_m', $id_service)
			->delete('pj_service_master');
	}

	function laporan_service($from, $to)
	{
		$sql = "
			SELECT 
	...
	{
		return $this->db->select('nomor_nota')->where('nomor_nota', $nota)->limit(1)->get('pj_service_master');
	}
}